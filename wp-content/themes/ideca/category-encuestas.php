		...
		<div>
			...
			<table class="table table-hover">
				<tbody>
		            ...
		            <?php 
		           		$nume = intval(preg_replace('/[^0-9]+/', '', get_the_content()), 10); 
		           		$myrows2 = $wpdb->get_results( "SELECT * FROM wp_poll_wp_Results WHERE Juna_IT_Poll_Add_Question_FieldID='".$nume."'" );
						//var_dump($myrows);
						$si2 = $myrows2[0]->Juna_IT_Poll_Count; // SI
						$no2 = $myrows2[1]->Juna_IT_Poll_Count; // NO
						$total2 = $si2+$no2;
						$para_si2 = number_format(($si2*100)/$total2,1,'.','');
						$para_no2 = number_format(($no2*100)/$total2,1,'.','');
 		            ?>                  	                            		            
		            <tr>
		            	<td>
		            		<div class="enc_img">
		            		<h6>Resultados</h6>
		            			<div class="progress">
								  	<div class="progress-bar para-si" role="progressbar" aria-valuenow="<?php echo $para_si2; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $para_si2; ?>%">
								    	<span class="sr-only"><?php echo $para_si2; ?> % Complete (warning)</span>
								    	<p>SI: <strong><?php echo $para_si2; ?></strong> %</p>
								  	</div>
								</div>
								<div class="progress">
								  	<div class="progress-bar para-no" role="progressbar" aria-valuenow="<?php echo $para_no2; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $para_no2; ?>%">
								    	<span class="sr-only"><?php echo $para_no2; ?>% Complete (warning)</span>
								    	<p>NO: <strong><?php echo $para_no2; ?></strong>%</p>
								  	</div>
								</div>
		            		</div>
		            	</td>
