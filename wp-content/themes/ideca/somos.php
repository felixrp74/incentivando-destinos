...
<?php if (have_posts()) : while (have_posts()) : the_post();?>
	<div class="container">	
		<div class="main-titulo fnd-celeste" ><h1><?php the_title(); ?></h1></div>		
			<div class="row">				
				<div class="col-sm-8 for-fotos">
					...
					for ($i = 1;$i<$conta;$i++) {
						?>
					<div class="cont-somos arreglo-somos">
						
							<?php
							$tit = explode("<hr />", get_the_content());
				echo wpautop($tit[0]);
//							echo '<h2 class="titulo_'.$i.'">'.$tit[0].'</h2>';
							...
							<?php } echo '</div>'; 
						}else{ echo '<p style="border:1px solid red">'.$tit[1].'</p>'; }
echo wpautop($tit[2]);;
							?>
					...
					<div>
	            		...
	            		<?php if($x%2==0) echo "<div class='clearfix'></div>"; endwhile; ?>
					</div>
				</div>
				<?php get_sidebar(); ?>
			</div>
